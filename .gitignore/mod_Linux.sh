...
geracheck ()
{
   ...
   showmount  | awk '{ print $1" "$3 }' 1>$CHKU_GFILE 

   nome_arquivo "pacotes_rpm"
   rpm -qia >$CHKU_GFILE 
    
   nome_arquivo "kernel_vigor"
   uname -sr >$CHKU_GFILE
   
   nome_arquivo "Servico_Boot"
   chkconfig --list >$CHKU_GFILE
   
   nome_arquivo "RumLevel_Configurado"
   awk '!/#|^ *$/ && /initdefault/' /etc/inittab >$CHKU_GFILE
 
   nome_arquivo "Configuracao_Boot"
   cat /etc/grub.conf |grep default >$CHKU_GFILE
